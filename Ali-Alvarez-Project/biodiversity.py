...
species.groupby('category').scientific_name.nunique().reset_index()    .sort_values(by='scientific_name',ascending=False)
...
species.groupby('conservation_status').scientific_name.nunique().reset_index()    .sort_values(by='scientific_name',ascending=False)
...
my_lambda = lambda x: True if x == 'No Intervention' else False
species['is_protected']=species.conservation_status.apply(my_lambda)
#checking it worked
species.head(10)
...
my_lambda = lambda x: True if 'Sheep' in x else False
...
print('Bryce:',int(bryce),'weeks' )
print('Yellowstone:',int(yellowstone),'weeks')
